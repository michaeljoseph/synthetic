...
def timesheet_from_standup(day):
    ...
    comments = standup_path.read_text().rstrip().split('\n')

    _date, _, hours = re.search(r'# ([0-9\-]*)\ ?(\[(\d*)\])?', comments[0]).groups()
    ...
    start = 9
    break_minutes = 60
    end = int(start + hours + (break_minutes / 60))
